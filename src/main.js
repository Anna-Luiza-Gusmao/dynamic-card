...
const cardNumberPattern = {
  ...
  dispatch: function(appended, dynamicMasked){
    const number = (dynamicMasked.value + appended).replace(/\D/g, "") //tudo que não é dígito é substituido por vazio

    //{regex} pega somente a propriedade regex do objeto
    //se a expressão regular está de acordo com o regex e vai encontra a máscara (sendo visa ou mastercard)
...
cardHolder.addEventListener("input", () => {
  ...
  ccHolder.innerText = cardHolder.value.length === 0 ? "FULADO DA SILVA" : cardHolder.value //if ternário
})
